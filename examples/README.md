...
The consumers use a local `replace` to `../rules` so the checkout is immediately editable. The module path remains `github.com/kakj-go/go-inject/examples/rules`. For a published-module consumer, remove that replace and require `v0.1.0-beta.3`.
